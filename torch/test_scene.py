...
parser.add_argument('--input_data_path', default='../mp_sdf_vox_2cm_input',required=False, help='path to input data')
parser.add_argument('--target_data_path', default='../mp_sdf_vox_2cm_target',required=False, help='path to target data')
parser.add_argument('--test_file_list',default='../filelists/mp-test-select.txt', required=False, help='path to file list of test data')
#parser.add_argument('--model_path',default='../sgnn.pth', required=False, help='path to model to test')
parser.add_argument('--model_path',default='../sgnn.pth', required=False, help='path to model to test')
parser.add_argument('--output', default='./test_scene_test', help='folder to output predictions')
# model params
...
# test params
parser.add_argument('--max_to_vis', type=int, default=20, help='max num to vis')
        ...
        for t, sample in enumerate(dataloader):
            ...
            if args.compute_pred_occs:
                ...
                for h in range(args.num_hierarchy_levels):
                    ...
                    mask_occ = (pred_occs[h][:, 0] < (dims[0]// factor)) & (pred_occs[h][:, 1] < (dims[1] // factor)) & (
                                pred_occs[h][:, 2] < (dims[2] // factor))
            ...
            data_util.save_predictions(output_vis, sample['name'], inputs, None, None, vis_pred_sdf,pred_occs, sample['world2grid'], args.truncation)
            num_vis += 1
...
def main():
    ...
    if len(test_files) > args.max_to_vis*2:
        test_files = test_files[args.max_to_vis:args.max_to_vis*2]
    else:
