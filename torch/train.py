...
def train(epoch, iter, dataloader, log_file, output_save):
    ...
    for t, sample in enumerate(dataloader):
        ...
        if compute_pred_occs:
            ...
            for h in range(args.num_hierarchy_levels):
                ...
                if len(output_occs[h][0]) == 0:
                    continue
                output_occs[h][1] = torch.nn.Sigmoid()(output_occs[h][1][:,0].detach()) > 0.4
        ...
        if output_visual:
            ...
            data_util.save_predictions(os.path.join(args.save, 'iter%d-epoch%d' % (iter, epoch), 'train'), sample['name'], inputs, target_for_sdf.cpu().numpy(), [x.cpu().numpy() for x in target_for_occs], vis_pred_sdf, pred_occs, sample['world2grid'].numpy(), args.truncation,args.vis_dfs)
